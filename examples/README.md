...
|---|---|---|
| `ts-quickstart/` | Use `@efs/sdk` from a TypeScript app (read a file, pin a file). | planned |
| `foundry-consumer/` | Import `@efs/solidity` into a Foundry project via remappings. | planned |
| `hardhat-consumer/` | Import `@efs/solidity` into a Hardhat project via `node_modules`. | planned |
